(() => {
  ...
  function bindGsap(lenis) {
    ...
    // Prefer a dedicated RAF loop so Lenis works even before GSAP ticker is busy.
    startRaf(lenis);
  }
  ...
  function initLenis() {
    ...
    // Recalculate after layout / loader / images settle.
    const refresh = () => {
      try {
        lenis.resize();
      } catch (_) {}
  ...
  // Init as soon as possible, then again on load if CDN was slow.
